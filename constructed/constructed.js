...
async function uploadDeck(cardList, setCode) {
    const response = await fetch("uploadDeck.php" + (setCode !== undefined ? "?set=" + setCode : ""), {
        method: "POST",
    ...
    if (response.ok) {
        if (text == "register") {
            ...
            //Click <a class='alert-link' href='preview/'>here</a> to see a preview
